...
function executeCommand(command: string) {
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      return { error: true, message: execError.stdout || execError.stderr };
    }
...
async function createDatabaseAndConfigure() {
  ...
  const updatedWranglerToml: toml.JsonMap = {
    d1_databases: [
      {
        binding: "DATABASE",
  ...
  try {
    const updatedToml = toml.stringify(updatedWranglerToml);
